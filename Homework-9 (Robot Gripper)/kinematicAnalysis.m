...
q = zeros(length(x0),length(time));
qd = zeros(length(x0),length(time));
qdd = zeros(length(x0),length(time));
...
%     qd(:,t) = Newton_Raphson(constEqsD,jacobFDiffD,x0d,tolerance,maxIterations);
    qd(:,t) = JacobianMatrix(InputParameters,q(:,t))\(-Ct(InputParameters,time(t)));
...
%     qdd(:,t) = Newton_Raphson(constEqsDD,jacobFDiffDD,x0dd,tolerance,maxIterations);
    qdd(:,t) = JacobianMatrix(InputParameters,q(:,t))\C_Gamma(InputParameters,q(:,t),qd(:,t),time(t));
